...
class MissionDefinition(BaseModel):
    ...
    def build_execution_manifests(self) -> "MissionDefinition":
        ...
        if not self.baseline_commands:
            object.__setattr__(self, "baseline_commands", [prepare, build_baseline, probe])
        ...
        if self.judge_manifest is None:
            apply = ["sh", "-lc", "git -C /opt/lda/work apply /opt/lda/input/candidate.patch && git -C /opt/lda/work add -A && git -C /opt/lda/work -c user.name=LDA -c user.email=lda@localhost commit -m candidate"]
            build_candidate = ["/opt/lda/harness/checks/build-generic-package.sh", "candidate", runtime]
            ...
            for name in (
                ...
                "install-paths", "precompiled-binary", "python-ctypes", "python-cffi", "rust-ffi",
            ...
            object.__setattr__(self, "judge_manifest", JudgeManifest(
                prepare_baseline=[CommandCheck(name="prepare-official-source", command=prepare), CommandCheck(name="build-official-baseline", command=build_baseline), CommandCheck(name="prepare-precompiled-probe", command=probe)],
                ...
                reverse_dependency_tests=[CommandCheck(name="reverse-dependency-link", command=["sh", "-lc", self.ffi_check_command])],
                application_smokes=[CommandCheck(name="application-smoke", command=["/opt/lda/fixtures/generic/probe", "1000"])],
