...
class RoleSession:
    ...
    def ask(self, prompt: str, *, schema=None):
        """One agent turn, retried in place on model-gateway failures.

        A gateway timeout or an empty reply is an infrastructure fact, not an
        answer. Retrying the same turn here (with a pause) keeps the sandbox,
        its setup and the run's kept state; only after the retries are spent
        does the failure surface as an InfrastructureOutage, which pauses the
        run for the driver loop to resume later.
        """
        attempts = max(1, int(os.getenv("LDA_TURN_RETRIES", "3")))
        pause = float(os.getenv("LDA_TURN_RETRY_PAUSE", "60"))
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                raise RuntimeError(f"agent turn failed: {detail[-1500:]}") from error
            text = "" if answer is None else str(answer)
            if text.strip():
                return text
            last = InfrastructureOutage("agent returned an empty answer")
            if attempt < attempts:
                time.sleep(pause * attempt)
