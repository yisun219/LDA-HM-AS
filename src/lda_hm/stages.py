...
# An agent CLI that cannot reach its model prints the transport failure as
# its "answer" and exits 0. Such text is evidence about the model gateway,
# never a Builder summary or a Reviewer verdict.
_API_ERROR_MARKERS = (
    re.compile(r"\AAPI Error\b"),
    re.compile(r'"cloudflare_error"\s*:\s*true'),
    re.compile(r"This error was generated by Cloudflare"),
    re.compile(r"safeguards flagged this message"),
    re.compile(r"Details:\s*`\[cyber\]`"),
    re.compile(r"\A(?:Error|error):?\s+(?:5\d\d|overloaded|rate.?limit)", re.IGNORECASE),
)


def api_error_marker(text: str) -> str:
    """Return the matched model-API failure marker in `text`, or ''."""
    stripped = text.strip()
    for pattern in _API_ERROR_MARKERS:
...
class HumanizeStages:
    ...
    def review_round(self, *, contract: str) -> ReviewResult:
        ...
        try:
            ...
            # A dead, killed, or unreachable Builder turn never produced a
            # candidate to judge. It is recorded as an infrastructure block:
            # no fence, benchmark, or stall counter may charge the candidate
            # for a model gateway that went away mid-turn.
            detail = ""
            if guard is not None and getattr(guard, "killed", False):
                detail = " (watchdog killed a stalled agent process)"
            turn_failure = f"BUILDER_TURN_FAILED{detail}: {error}"
            builder_text = turn_failure
        else:
            marker = api_error_marker(builder_text)
            if marker:
                # The CLI printed a transport failure as its answer.
                turn_failure = f"BUILDER_TURN_FAILED (model API error: {marker}): {builder_text[:600]}"
                builder_text = turn_failure
        ...
        if turn_failure:
            return self._blocked("builder-infra", turn_failure, infra=True)
    ...
    def _prior_feedback(self) -> str:
        ...
        for name in ("blocked.json", "review.json"):
            ...
            if name == "blocked.json":
                ...
                if source == "builder-infra":
                    return (
                        "your previous turn was interrupted by an infrastructure "
                        "failure (model gateway or sandbox transport), not by a "
                        "verdict on your work. The worktree may hold uncommitted "
                        "work from that turn: inspect `git status` and `git diff`, "
                        "finish or discard it deliberately, commit, and continue "
                        f"the round contract. Recorded failure: {value.get('reason', '')[:300]}"
                    )
                return f"{source}: {value.get('reason', '')}"
    ...
    def _review_result(answer: object) -> ReviewResult:
        text = answer if isinstance(answer, str) else str(answer)
        marker = api_error_marker(text)
        if marker:
            raise RuntimeError(f"reviewer answer is a model API failure ({marker})")
        lines = [line.strip() for line in text.splitlines() if line.strip()]
        # The protocol block is the LAST one in the answer: a reviewer that
        # restates the protocol while reasoning, then closes with its real
        # verdict, is answering correctly. Only the closing block is parsed.
        starts = [index for index, line in enumerate(lines) if line.startswith("VERDICT:")]
        if not starts:
            raise ValueError("reviewer must end with a VERDICT line")
        block = lines[starts[-1]:]
        verdict_lines = [line for line in block if line.startswith("VERDICT:")]
        status_lines = [line for line in block if line.startswith("STATUS:")]
        blocking_lines = [line for line in block if line.startswith("BLOCKING:")]
        if len(verdict_lines) != 1:
            raise ValueError("reviewer must return exactly one VERDICT line")
        if len(status_lines) != 1:
            raise ValueError("reviewer must return exactly one STATUS line in its closing protocol block")
        if not blocking_lines:
            raise ValueError("reviewer must return at least one BLOCKING line in its closing protocol block")
        try:
