...
bootstrap_python="${LDA_BOOTSTRAP_PYTHON:-python3}"

mkdir -p "$run_root"

if ! "$bootstrap_python" -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 12) else 1)'; then
  printf '%s bootstrap_python_invalid=%s required=3.12+\n' \
    "$(date -Iseconds)" "$bootstrap_python" >> "$log_path"
  exit 2
fi
...
while true; do
  ...
  if [[ "$http_code" == "200" ]]; then
    if [[ -f "$run_root/.lda/controller.json" ]]; then
      printf '%s lda_action=resume\n' "$(date -Iseconds)" >> "$log_path"
      PYTHONPATH=src "$bootstrap_python" ./lda --root "$run_root" resume \
        --run-id "$run_id" >> "$log_path" 2>&1
    else
      printf '%s lda_action=run\n' "$(date -Iseconds)" >> "$log_path"
      PYTHONPATH=src "$bootstrap_python" ./lda --root "$run_root" run \
        --flow argus-humanize \
        --run-id "$run_id" \
