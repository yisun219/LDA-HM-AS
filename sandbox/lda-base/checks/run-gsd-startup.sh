...
test -n "$program"
run() { for _ in $(seq 1 20); do lda_run_with_pkg "$mode" timeout 12 "$program" --version 2>&1 || true; done | sha256sum | cut -c1-16; }
run >/dev/null
lda_bench_run micro plugin-version "$mode" 20 run
