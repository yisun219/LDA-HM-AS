...
test -x "$program"
run() { out="${LDA_REMOTE_TMPDIR:-/scratch/lda-hm}/lo-micro-$mode"; rm -rf "$out"; mkdir -p "$out"; lda_run_with_pkg "$mode" "$program" --headless --convert-to pdf --outdir "$out" "$TOP10_FIXDIR/sample.fodt" >/dev/null; pdftotext "$out/sample.pdf" - | sha256sum | cut -c1-16; rm -rf "$out"; }
run >/dev/null
