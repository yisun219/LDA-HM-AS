...
# libpng-card known-bad-sample validation (run via the card's
# selfcheck_commands): each probe feeds a deliberately wrong input to a
# fence primitive used by this card and requires it to fail.
