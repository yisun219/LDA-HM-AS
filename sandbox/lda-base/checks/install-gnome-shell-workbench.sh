...
sudo -n apt-get "${OPTS[@]}" install -y --allow-downgrades --no-install-recommends \
  gnome-shell gnome-shell-common mutter-18-tests python3-dbusmock python3-dbus python3-gi \
  libgl1-mesa-dri libegl-mesa0 gsettings-desktop-schemas dconf-gsettings-backend dbus-daemon
/opt/lda/harness/checks/prepare-gnome-shell-fixtures.sh
echo "gnome-shell workbench installed (runtime, mock session runner, fixtures)"
