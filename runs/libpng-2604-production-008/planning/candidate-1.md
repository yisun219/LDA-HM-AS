The review converges on the primary direction and one blocking wording fix (N1/P3), plus two optional improvements I'll adopt. Here is the revised plan.

---

# Candidate Plan (Rev 2) — libpng Decode Optimization, Round 0 (Ubuntu 26.04)
...
> **Independent review incorporated (Rev 2):** the review verified both artifacts against this plan and found the primary direction, scope, rejections, retained alternatives, and constraint set all correct. One **required change** (N1/P3 wording) is resolved below; two **optional improvements** (build-path pre-decision, concrete unreachability check) are adopted. The earlier independent-analysis API error is downgraded to a non-blocking process note per the review.
...
1. **T1 — Pre-apply base verification.** Confirm the five target files match the diff's `index`/context hunks (partially done in Rev 1; extend to the `filter_sse2_intrinsics.c` sub4 anchor and the pngpriv.h function-decl anchor). Abort if any hunk context mismatches.
...
4. **T4 — Dispatch sanity inspection.** Confirm: (a) `have_sse41` is a function-local `static int … = -1` resolved once via a single `__builtin_cpu_supports` guarded by `if (have_sse41 < 0)`; (b) the **only** references to `png_read_filter_row_paeth{3,4}_sse41` are the two `if (have_sse41)` assignments in `intel_init.c`; (c) Up + Paeth wiring is present for both bpp==3 and bpp==4.
...
- **P3 (dispatch executes once — REVISED):** The SSE4.1 CPUID probe **executes at most once per process**. The invariant is on *execution*, not textual location: the patch intentionally places `__builtin_cpu_supports("sse4.1")` **inside** `png_init_filter_functions_sse2` (a per-image init function, patch lines 262–265), guarded by a function-local `static int have_sse41 = -1` with `if (have_sse41 < 0)`. This textual placement is **expected, certified, and correct** — the static guard ensures the CPUID runs only on the first call for the process lifetime. Verification confirms the static-guard structure is intact; it does **not** require (and must not induce) moving the call out of the init function.
...
## 5a. Build-Path Decision (resolved per review)

**Decision:** Round 0 builds via the **CMake** path. Rationale: the patch touches only `CMakeLists.txt`; `PNG_INTEL_SSE41_RUNTIME` auto-defines on GCC/x86 (pngpriv.h lines 317–323), so `intel_init.c` references `png_read_filter_row_paeth{3,4}_sse41` unconditionally, and the new TU is compiled only under CMake's updated source list. An **Autotools** build (which the patch does not update) would therefore fail at **link time** with an undefined reference — not at compile time. **Abort-and-report condition:** if the authoritative package build is Autotools and a link failure appears, stop and report; do **not** improvise a `Makefile.am` edit (that would breach the verbatim mandate). The prior certification succeeded, confirming CMake is a valid path.
...
2. **Authoritative build system (working assumption set, confirmation welcome).** Working assumption is **CMake** (see §5a). If the user knows the Ubuntu 26.04 package build is Autotools-authoritative, flag before T3 so the link-failure abort condition is anticipated rather than discovered.
3. **Independent-analysis API error (process note only, non-blocking).** The Rev-1 analysis field returned a `[cyber]` safeguard false-positive with no technical content; the independent review has since supplied a full artifact-verified assessment, so technical grounding is confirmed. No action needed unless the user wants the analysis stage re-run for the record.
...
The single **REQUIRED_CHANGE** from the review — rewording N1/P3 so the once-per-process **execution** invariant is not misread as forbidding the guarded CPUID's textual presence in `png_init_filter_functions_sse2` — is resolved in §3 (P3) and §4 (N1). Both optional improvements (CMake build-path pre-decision in §5a; concrete `paeth*_sse41` unreachability check in N2/T4) are adopted. Remaining items in §6 are non-blocking human decisions retained explicitly.
